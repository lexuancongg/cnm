...
const baseUrl = 'http://localhost:8000/api/backoffice/categories';
...
export async function getProductsByCategory(
  ...
  const url = `http://localhost:8000/api/product/backoffice/category/${categorySlug}/products?pageNo=${pageNo}`;
  return (await apiClientService.get(url)).json();
}
