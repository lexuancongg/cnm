...
import type { AuthorVm } from '../../../modules/catalog/models/Brand';
...
const ProductList: NextPage = () => {
    ...
    deleteProduct(productIdWantToDelete)
      .then((response) => {
        ...
        getProducts(pageNo, productName, brandName).then((data) => {
          ...
          setProducts(data.productPreviewsPayload? data.productPreviewsPayload : []);
          setLoading(false);
        });
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    getProducts(pageNo, productName, brandName).then((data) => {
      setTotalPage(data.totalPages);
      setProducts(data.productPreviewsPayload);
      setLoading(false);
    }).finally(() => {
      setLoading(false);
    });
